...
export class AgentConnectionStatus extends LitElement {
  ...
  statusToText(status?: ConnectionStatus) {
    ...
    switch (status.type) {
      ...
      case 'InitSent':
        return `waiting for init accept${status.attemptCount && status.attemptCount > 1 ? `(attempt #${status.attemptCount})` : ''}...`;
      case 'AcceptSent':
        return `waiting for SDP exchange${status.attemptCount && status.attemptCount > 1 ? `(attempt #${status.attemptCount})` : ''}...`;
      case 'SdpExchange':
